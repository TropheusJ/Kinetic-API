...
public abstract class KineticTileEntity extends SmartTileEntity
	implements Tickable, IHaveGoggleInformation, IHaveHoveringInformation, IInstanceRendered {
	...
	public float calculateStressApplied() {
		float impact = (float) AllConfigs.SERVER.kinetics.stressValues.getImpactOf(getCachedState().getBlock());
	...
	public void onSpeedChanged(float previousSpeed) {
		...
		if (fromOrToZero && previousSpeed == 0 && !world.isClient)
			AllTriggers.getPlayersInRange(world, pos, 4)
				.forEach(p -> AllTriggers.KINETIC_BLOCK.trigger(p, getCachedState()));
	}
