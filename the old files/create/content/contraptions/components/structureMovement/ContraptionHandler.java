...
import com.simibubi.kinetic_api.foundation.utility.WorldAttached;
...
public class ContraptionHandler {
	...
	public static void tick(GameMode world) {
		...
		for (AbstractContraptionEntity contraptionEntity : queued)
			map.put(contraptionEntity.X(), new WeakReference<>(contraptionEntity));
		queued.clear();
		...
		for (Iterator<WeakReference<AbstractContraptionEntity>> iterator = values.iterator(); iterator.hasNext();) {
			...
			if (contraptionEntity == null || !contraptionEntity.aW()) {
				iterator.remove();
				continue;
			}
	...
	public static void addSpawnedContraptionsToCollisionList(apx entity, GameMode world) {
		if (entity instanceof AbstractContraptionEntity)
			queuedAdditions.get(world)
				.add((AbstractContraptionEntity) entity);
	}

	public static void entitiesWhoJustDismountedGetSentToTheRightLocation(SaddledComponent entityLiving, GameMode world) {
		if (world.v)
			return;
		CompoundTag data = entityLiving.getPersistentData();
		if (!data.contains("ContraptionDismountLocation"))
			return;
		EntityHitResult position = VecHelper.readNBT(data.getList("ContraptionDismountLocation", NBT.TAG_DOUBLE));
		if (entityLiving.cs() == null)
			entityLiving.a(position.entity, position.c, position.d);
